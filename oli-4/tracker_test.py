...
class NaoTrackerDemo(SICApplication):
    ...
    def run(self):
        try:
            ...
            # Make NAO sit before starting
            self.logger.info("Putting NAO into Sit posture...")
            self.nao.motion.request(NaoPostureRequest("Sit", 0.5))
            time.sleep(1)

            self.logger.info("===== Starting Sitting Gesture Test =====")

            for category, gesture_list in self.sitting.items():
            ...
            self.logger.info("===== Finished Sitting Gesture Test =====")
            print("All gestures tested. You can now mark which ones include audio.")
